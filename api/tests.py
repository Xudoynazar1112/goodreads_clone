...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username='xudoynazar', first_name="Xudoynazar", last_name="Saparov",
                                              email="saparovxudoynazar@gmail.com")
        self.user.set_password("something")
        self.user.save()
    ...
    def test_review_detail(self):
        ...
        br = Review.objects.create(book=book, user=self.user, stars_given=5, comment="Very good book")
        ...
        self.assertEqual(response.data['user']['first_name'], "Xudoynazar")
        self.assertEqual(response.data['user']['username'], "xudoynazar")
        self.assertEqual(response.data['user']['last_name'], "Saparov")
    ...
    def test_delete_review(self):
        ...
        self.assertFalse(Review.objects.filter(id=br.id).exists())
    ...
    def test_create_review(self):
        ...
        br = Review.objects.get(book=book)
    ...
    def test_review_list(self):
        book = Book.objects.create(title="Book1", description="Description1", isbn="123456")
        br = Review.objects.create(book=book, user=self.user, stars_given=5, comment="Very good book")
        br_two = Review.objects.create(book=book, user=self.user, stars_given=3, comment="Not good")
